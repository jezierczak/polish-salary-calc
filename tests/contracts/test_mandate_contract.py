...
def test_mandate_contract_common_with_and_without_fp_fgsp_ppk(
    ...
    assert mc.is_calculated == True
    assert (
        mc.fp == salary_base * rates.fp_rate
        if salary_base > rates.minimum_wage
        else mc.fp == Decimal("0")
    )
    ...
    assert (
        mc.employee_ppk_contribution
        == Decimal("6000") * mc.contract_settings.employee_ppk
    )
    assert (
        mc.employer_ppk_contribution
        == Decimal("6000") * mc.contract_settings.employer_ppk
    )
