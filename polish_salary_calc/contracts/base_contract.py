...
class BaseContract[T: ContractSettngs](Salary, ABC):
    ...
    def calculate_social_security_base_total(self) -> Decimal:
        ...
        return (
            self.contract_settings.social_security_base_sum + self.social_security_base
        )
    ...
    def calculate_social_insurance_sum(self) -> Decimal:
        ...
        return (
            self.pension_insurance + self.disability_insurance + self.sickness_insurance
        )
    ...
    def calculate_ppk_tax(self) -> Decimal:
        ...
        return (
            self.social_security_base
            * self.contract_settings.employer_ppk
    ...
    def calculate_accident_insurance(self) -> Decimal:
        ...
        return (
            self.social_security_base * self.contract_settings.accident_insurance_rate
        )

    def calculate_fp(self) -> Decimal:
        """Labor Fund contribution (only if gross salary >= minimum wage)."""
        if (
            self.contract_settings.current_month_gross_sum + self.salary_gross
            >= self.rates.minimum_wage
        ):
    ...
    def calculate(
        self, salary_base: Decimal, salary_type: SalaryType = SalaryType.GROSS
    ) -> None:
        """
        ...
        if self.contract_settings is None:
            raise AttributeError(
                'No contract_settings set to contract, use "update_options" before calculating'
            )
